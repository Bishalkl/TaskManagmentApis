...
migrate-up-to:
	...
	@export $$(grep -v '^\s*#' .env | grep -v '^\s*$$' | xargs) && \
	goose -dir $(MIGRATION_DIR) $(DB_DRIVER) $(DB_STRING) up $(version)

migrate-down-to:
	#make migrate-down-to version=2
	@echo "⬇️ Rolling down to version $(version)"
	@export $$(grep -v '^\s*#' .env | grep -v '^\s*$$' | xargs) && \
	goose -dir $(MIGRATION_DIR) $(DB_DRIVER) $(DB_STRING) down $(version)
...
migrate-version:
	...
	@export $$(grep -v '^\s*#' .env | grep -v '^\s*$$' | xargs) && \
	goose -dir $(MIGRATION_DIR) $(DB_DRIVER) $(DB_STRING) version
